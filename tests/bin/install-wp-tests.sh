...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	# http serves a hierarchical list of available builds
...
install_wp() {
	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		if [ $WP_VERSION == 'latest' ]; then
			...
			# https serves a hierarchical list of available builds
			download https://api.wordpress.org/core/version-check/1.7/ $TMPDIR/wp-latest.json
			if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
				# version x.x.0 means the first release of the major version, so strip off the .0 and download version x.x
				local LATEST_VERSION=${WP_VERSION%??}
			else
				# otherwise, scan the releases and get the most current micro version
