...
abstract class Xmoney_Payments_List_Table {
	 ...
	 * @var array
	 */
	protected array $compat_methods = array(
		'set_pagination_args',
		'get_views',
		'get_bulk_actions',
		'bulk_actions',
	...
	public function __construct( $args = array() ) {
		...
		if ( empty( $this->modes ) ) {
			$this->modes = array(
				'list'    => esc_html__( 'List View', 'xmoney-payments' ),
				'excerpt' => esc_html__( 'Excerpt View', 'xmoney-payments' ),
			);
		}
	}
	...
	public function no_items() {
		esc_html_e( 'No items found.', 'xmoney-payments' );
	}
	...
	protected function bulk_actions( string $which = '' ) {
		...
		echo '<option value="-1">' . esc_html__( 'Bulk Actions', 'xmoney-payments' ) . "</option>\n";
		...
		submit_button( esc_attr__( 'Apply', 'xmoney-payments' ), 'action', '', false, array( 'id' => "doaction$two" ) );
		echo "\n";
	}
	...
	protected function row_actions( array $actions, bool $always_visible ): string {
		...
		$out .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . esc_html__( 'Show more details', 'xmoney-payments' ) . '</span></button>';
	...
	protected function months_dropdown( string $post_type ) {
		...
		?>
			...
			<select name="m" id="filter-by-date">
				<option<?php selected( $m, 0 ); ?> value="0"><?php esc_html__( 'All dates', 'xmoney-payments' ); ?></option>
			<?php
	...
	protected function comments_bubble( int $post_id, int $pending_comments ) {
		...
		$approved_only_phrase = sprintf( _n( '%s comment', '%s comments', $approved_comments, 'xmoney-payments' ), esc_attr( $approved_comments_number ) );
		/* translators: 1: Singular number of approved comments, 2: Plural number of approved comments. */
		$approved_phrase = sprintf( _n( '%s approved comment', '%s approved comments', $approved_comments, 'xmoney-payments' ), esc_attr( $approved_comments_number ) );
		/* translators: 1: Singular number of pending comments, 2: Plural number of pending comments. */
		$pending_phrase = sprintf( _n( '%s pending comment', '%s pending comments', $pending_comments, 'xmoney-payments' ), esc_attr( $pending_comments_number ) );

		// No comments at all.
		if ( ! $approved_comments && ! $pending_comments ) {
			printf(
				'<span aria-hidden="true">Ñ</span><span class="screen-reader-text">%s</span>',
				esc_html__( 'No comments', 'xmoney-payments' )
			);
			...
			printf(
				...
				esc_attr( $approved_comments_number ),
				$pending_comments ? esc_html__( 'No approved comments', 'xmoney-payments' ) : esc_html__( 'No comments', 'xmoney-payments' )
			);
		}
		...
		if ( $pending_comments ) {
			...
			printf(
				...
				esc_attr( $pending_comments_number ),
				$approved_comments ? esc_html__( 'No pending comments', 'xmoney-payments' ) : esc_html__( 'No comments', 'xmoney-payments' )
			);
		}
	}
	...
	protected function pagination( string $which ) {
		...
		if ( $disable_first ) {
			...
			$page_links[] = sprintf(
				...
				esc_html__( 'First page', 'xmoney-payments' ),
				'&laquo;'
			);
		}
		...
		if ( $disable_prev ) {
			...
			$page_links[] = sprintf(
				...
				esc_html__( 'Previous page', 'xmoney-payments' ),
				'&lsaquo;'
			);
		}
		...
		if ( 'bottom' === $which ) {
			...
			$total_pages_before = '<span class="screen-reader-text">' . esc_html__( 'Current Page', 'xmoney-payments' ) . '</span><span id="table-paging" class="paging-input"><span class="tablenav-paging-text">';
		} else {
			$html_current_page = sprintf(
				"%s<input class='current-page' id='current-page-selector' type='text' name='paged' value='%s' size='%d' aria-describedby='table-paging' /><span class='tablenav-paging-text'>",
				'<label for="current-page-selector" class="screen-reader-text">' . esc_html__( 'Current Page', 'xmoney-payments' ) . '</label>',
				esc_attr( $current ),
				strlen( $total_pages )
			);
		}
		...
		if ( $disable_next ) {
			...
			$page_links[] = sprintf(
				...
				esc_html__( 'Next page', 'xmoney-payments' ),
				'&rsaquo;'
			);
		}
		...
		if ( $disable_last ) {
			...
			$page_links[] = sprintf(
				...
				esc_html__( 'Last page', 'xmoney-payments' ),
				'&raquo;'
			);
		}
	...
	public function print_column_headers( bool $with_id = true ) {
		...
		if ( ! empty( $columns['cb'] ) ) {
			static $cb_counter = 1;
			$columns['cb']     = '<label class="screen-reader-text" for="cb-select-all-' . esc_attr( $cb_counter ) . '">' . esc_html__( 'Select All', 'xmoney-payments' ) . '</label>'
	...
	public function display() {
		...
		?>
			<table class="wp-list-table <?php echo esc_attr( $this->get_table_classes() ); ?>">
				<thead>
				<tr>
	...
	public function ajax_response() {
		...
		if ( isset( $this->_pagination_args['total_items'] ) ) {
			...
			/* translators: 1: Singular number of items, 2: Plural number of items. */
				_n( '%s item', '%s items', $this->_pagination_args['total_items'], 'xmoney-payments' ),
